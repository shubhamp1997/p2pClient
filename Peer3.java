...
public class Peer3 {
        ...
        public void run() {
            ...
            if (!done) {
                ...
                try {
                    ...
                    peerbyteStream.write(choke);
                    byte[] chokeMsg = peerbyteStream.toByteArray();

                    byte[] unchoke = ByteBuffer.allocate(1).put(messageTypeMap.get("unchoke")).array();

                    peerbyteStream.reset();
                    peerbyteStream.write(messageLength);
                    peerbyteStream.write(unchoke);
                    byte[] unchokeMsg = peerbyteStream.toByteArray();
                ...
                for (int i = 0; i < numNeighbors; i++) {
                    if (!toUnchoke.isEmpty()) {
                        ...
                        Socket randomSocket = (Socket) sockets[random.nextInt(sockets.length)];
    ...
    static class OptomisticUnchoke extends TimerTask {
        ...
        public void run() {
            ...
            Socket toUnchoke = (Socket) pN[random.nextInt(pN.length)];
